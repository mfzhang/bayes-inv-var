function [ param, mu_q, sigmaPred, nelbo err] = varLinearIterativeGauss( y, param, rbconf)
...
while ((i < rbconf.maxiter) && (err(i) > rbconf.tol))
    ...
    %% Update Precision-adjusted mean
    % nu_q     = nu_p + J'*lambday*(y - fwdval + J*mu_q);
    nu_q  = updateMeanVarGauss( mu_q, nu_p, fwdval, J, y, lambday, cholL_q, 1, 1e-3);
